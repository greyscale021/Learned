...
1. This documentary is only useful after we have finished a basic tutorial of git.
1. Most Git commands act on our current branch by default.
2. Terms:
    - HEAD: The last local commit on our current branch.
    - Staging Area: What we’ve added with git add.
    - Working Directory: What’s currently in our editor.
    - Current branch: The branch we are currently in.
...
This section covers the basic commands we’ll need to initialize our own local Git repository and set it up.

- **Set up Git identity** (optional, but recommended before committing to keep track of who made each change).
    ```bash
    git config --global user.name "Your Name"
    git config --global user.email "you@example.com"
    ```

- **Initialize a new Git repository** in our project folder.
    ```bash
    git init
    ```

- **Check the status of our files** (shows modified/unstaged files).
    ```bash
    git status
    ```
...
Once we’ve set up our local repository, we can start tracking and saving our changes. These commands will allow us to see which files have been modified and save those changes locally.
...
- **Stage files** (prepare them for committing).
    ...
    - Stage:
      ...
      git add .   # Stage the current and all it's sub directories
      ```
    - Unstage:
      ```bash
      git restore --staged <file-name>   # Unstage a specific file
      git restore --staged .   # Unstage the current and all it's sub directories
      ```
...
- **Diff** (Will show the difference):
    ...
    - HEAD: Snapshot of the last commit on our current branch.
    - Staging Area: What we’ve added with git add.
    - Working Directory: What’s currently in our editor.
  ```bash
  ...
  ```
    ...
    git diff HEAD..origin/main   # HEAD vs origin/main, How to turn the local head into origin main/remote repo.
    ```
...
- **Stashing Changes**:
    ...
    allowing us to switch branches or work on something else.
    ```bash
...
- Good practice in pro workflow: Reason being when collaborating rebase can cause history confusion among different contriburtors.
    ...
    git stash   # If we have uncommitted work
    git pull --rebase
    git stash pop       
    ```
    ```bash
...
Branching allows us to work on separate features or fixes in your codebase without affecting the main branch. This is crucial when working on multiple tasks simultaneously.
...
- **Connect local repository to a remote hub** (GitHub, GitLab, etc.):
    ```bash
...
- Or, **Clone** (download) a remote repo to local:
    ...
    ```
- **Push** (Upload) code to the remote repository (this uploads the changes as branch):
    ```bash
...
- **Fetch** (Retrieve changes from remote without changing your local branches):
    ```bash
    git fetch origin   # Updates our local remote head(origin/main).
    ```
- **Pull** (Update) the latest changes from the remote repository (update your local repository with changes from the remote):
    ...
    ```
    ```bash
    git pull   # If we already have set upstream for the current branch.
    ```
    ```bash
...
Sometimes, we need to **undo** or **reset** changes either locally or even specific commits. This section deals with undoing mistakes and managing our commits effectively.
...
- **Reset**: An old and versitile cmd.
    ...
    git reset --hard   # Resets staging & working dir to match last commit\HEAD
    ```
...
Viewing the history of commits allows us to track changes over time.
